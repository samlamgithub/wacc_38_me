...
.L2:
	...
	add	sp, fp, #0*/
		...
		String label1 = labelMan.nextLabel(); // label for the body
		String label0 = labelMan.nextLabel(); //label for expression
		insMan.addInstruction("B " + label1);
		insMan.addInstruction(label1 + ":");
	...
	public String visitIfthenelse(@NotNull BasicParser.IfthenelseContext ctx) {
		...
		String label0 = labelMan.nextLabel();
		String label1 = labelMan.nextLabel();
		boolean isThen = ctx.statement().size() > 1;
		insMan.addInstruction(new Instruction("CMP", reg, "#0"));
		insMan.addInstruction(new Instruction("BEQ", isThen ? label0 : label1));
		regMan.setFree(reg);
